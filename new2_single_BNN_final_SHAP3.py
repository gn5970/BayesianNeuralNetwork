...
from torch.nn import Module, Parameter
from captum.attr import IntegratedGradients, DeepLift, GradientShap,KernelShap, GuidedBackprop, LRP
from torch.nn.modules.utils import _single, _pair, _triple
from torch.autograd import grad
import sys
import shap
...
Z0_shap_24_1=np.zeros((264,members[0]))
Z1_shap_24_1=np.zeros((264,members[0]))
Z2_shap_24_1=np.zeros((264,members[0]))
Z3_shap_24_1=np.zeros((264,members[0]))
Z4_shap_24_1=np.zeros((264,members[0]))
Z5_shap_24_1=np.zeros((264,members[0]))
Z6_shap_24_1=np.zeros((264,members[0]))

Z0_shap_96_1=np.zeros((264,members[0]))
Z1_shap_96_1=np.zeros((264,members[0]))
Z2_shap_96_1=np.zeros((264,members[0]))
Z3_shap_96_1=np.zeros((264,members[0]))
Z4_shap_96_1=np.zeros((264,members[0]))
Z5_shap_96_1=np.zeros((264,members[0]))
Z6_shap_96_1=np.zeros((264,members[0]))

Z0_shap_24_2=np.zeros((264,members[0]))
Z1_shap_24_2=np.zeros((264,members[0]))
Z2_shap_24_2=np.zeros((264,members[0]))
Z3_shap_24_2=np.zeros((264,members[0]))
Z4_shap_24_2=np.zeros((264,members[0]))
Z5_shap_24_2=np.zeros((264,members[0]))
Z6_shap_24_2=np.zeros((264,members[0]))

Z0_shap_96_2=np.zeros((264,members[0]))
Z1_shap_96_2=np.zeros((264,members[0]))
Z2_shap_96_2=np.zeros((264,members[0]))
Z3_shap_96_2=np.zeros((264,members[0]))
Z4_shap_96_2=np.zeros((264,members[0]))
Z5_shap_96_2=np.zeros((264,members[0]))
Z6_shap_96_2=np.zeros((264,members[0]))

Z0_shap_24_3=np.zeros((264,members[0]))
Z1_shap_24_3=np.zeros((264,members[0]))
Z2_shap_24_3=np.zeros((264,members[0]))
Z3_shap_24_3=np.zeros((264,members[0]))
Z4_shap_24_3=np.zeros((264,members[0]))
Z5_shap_24_3=np.zeros((264,members[0]))
Z6_shap_24_3=np.zeros((264,members[0]))

Z0_shap_96_3=np.zeros((264,members[0]))
Z1_shap_96_3=np.zeros((264,members[0]))
Z2_shap_96_3=np.zeros((264,members[0]))
Z3_shap_96_3=np.zeros((264,members[0]))
Z4_shap_96_3=np.zeros((264,members[0]))
Z5_shap_96_3=np.zeros((264,members[0]))
Z6_shap_96_3=np.zeros((264,members[0]))
...
#    ts[1:] = ts_diff[1:] + ts_base[:-1]
#    return ts.tolist() 
def calculate_integrated_gradients_constbaseline(model, input_sequence, targets,num_steps=100):
    ...
    for target in targets:
        attr = ig.attribute(input_sequence, baselines=baseline_sequence, n_steps=num_steps, target=target)
        attributions.append(attr.cpu().detach().numpy())
    return np.array(attributions)

def calculate_gradientshap_constbaseline(model, input_sequence, targets):
    gs = GradientShap(model)
    #baseline_sequence = torch.full(input_sequence.size(), constant, device=input_sequence.device)
    baseline_sequence = torch.normal(mean=0.0, std=1.0, size=input_sequence.size(), device=input_sequence.device)
    attributions = []
    for target in targets:
        attr = gs.attribute(input_sequence, baselines=baseline_sequence, target=target)
        attributions.append(attr.cpu().detach().numpy())
    return np.array(attributions)


skill=np.zeros((9,10))
IG_NADW1=np.zeros((4,264,3))
IG_LCDW1=np.zeros((4,264,3))
IG_UCDW1=np.zeros((4,264,3))

IG_NADW2=np.zeros((4,264,3))
IG_LCDW2=np.zeros((4,264,3))
IG_UCDW2=np.zeros((4,264,3))
...
for r in range(10):
 ...
 for w in range(1):
    ...
    if w ==0:
        ...
        #torch.cuda.manual_seed_all(seed)
        data = io.loadmat('data_UCDW.mat')
        # Extract variables X and y

        thetao_index_UCDW = data['thetao_index_UCDW']
        thetao_index_UCDW=thetao_index_UCDW.reshape(1980,1)
        so_index_UCDW = data['so_index_UCDW']
        so_index_UCDW=so_index_UCDW.reshape(1980,1)
        eke_index_UCDW = data['eke_index_UCDW']
        eke_index_UCDW=eke_index_UCDW.reshape(1980,1)
        N_index_UCDW = data['N_index_UCDW']
        N_index_UCDW=N_index_UCDW.reshape(1980,1)

        T = data['T']
        D=np.concatenate((thetao_index_UCDW,so_index_UCDW,eke_index_UCDW,N_index_UCDW),axis=1)
        T=T.reshape(1980,1) 
        D2=np.zeros((1980,4))
    ...
    # Lists to hold the training and validation losses for each fold
    all_training_losses = []
    ...
    avg_training_loss = [sum(x) / k_folds for x in zip(*all_training_losses)]
    avg_validation_loss = [sum(x) / k_folds for x in zip(*all_validation_losses)]

    plt.figure()
    plt.plot(avg_training_loss)
    plt.plot(avg_validation_loss)
    ...
    for N in range(264):
       if N==0: 
            ...
            gs_ts1 = calculate_gradientshap_constbaseline(model_ts1, x_test, targets=targets)     
        
            ig_attributions_all1.append((gs_ts1))
    ...
    Z0_shap_24_1[:,r]=ig_attributions_all1[:,-1,-72,0]
    Z1_shap_24_1[:,r]=ig_attributions_all1[:,-1,-72,1]
    Z2_shap_24_1[:,r]=ig_attributions_all1[:,-1,-72,2]
    Z3_shap_24_1[:,r]=ig_attributions_all1[:,-1,-72,3]

    Z0_shap_96_1[:,r]=ig_attributions_all1[:,-1,-1,0]
    Z1_shap_96_1[:,r]=ig_attributions_all1[:,-1,-1,1]
    Z2_shap_96_1[:,r]=ig_attributions_all1[:,-1,-1,2]
    Z3_shap_96_1[:,r]=ig_attributions_all1[:,-1,-1,3]
 
            #y_test=y_test[:,:,0] 


    A_mean_baseline1_24_1[:,r,:]=np.concatenate((Z0_shap_24_1[:,r,np.newaxis],Z1_shap_24_1[:,r,np.newaxis],Z2_shap_24_1[:,r,np.newaxis],Z3_shap_24_1[:,r,np.newaxis]),axis=1)
    A_mean_baseline1_96_1[:,r,:]=np.concatenate((Z0_shap_96_1[:,r,np.newaxis],Z1_shap_96_1[:,r,np.newaxis],Z2_shap_96_1[:,r,np.newaxis],Z3_shap_96_1[:,r,np.newaxis]),axis=1)
...
for w in range(1):
    w=0
    plt.figure()
    plt.bar(feature,np.nanmean(np.nanmean(A_mean_baseline1_24_1[:,:,:],axis=0),axis=0).reshape(4,), yerr=stds_24_1, capsize=5, color='skyblue', edgecolor='black')
    ...
    plt.savefig("shap_composite_2_UCDW_"+str(w)+".png")



    plt.figure()
    plt.bar(feature,np.nanmean(np.nanmean(A_mean_baseline1_96_1[:,:,:],axis=0),axis=0).reshape(4,), yerr=stds_96_1, capsize=5, color='skyblue', edgecolor='black')
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    # Set y-axis to log scale
    plt.ylabel('SHAP',fontsize=13)
    plt.xlabel('Input feature',fontsize=13)
    ...
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.savefig("shap_composite_8_UCDW_"+str(w)+".png")
...
for w in range(1):
    ...
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,A_mean_baseline1_24_1[:,:,0])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_PT',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Input feature',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,A_mean_baseline1_24_1[:,:,1])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_Salinity',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.savefig("shap_composite_2_UCDW_time_"+str(w)+".png")

    w=1
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,A_mean_baseline1_24_1[:,:,2])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_EKE',fontsize=13)
    # Set y-axis to log scale
    #plt.xlabel('Input feature',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,A_mean_baseline1_24_1[:,:,3])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_N',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
...
for w in range(1):
    ...
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,A_mean_baseline1_96_1[:,:,0])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_PT',fontsize=13)
    # Set y-axis to log scale
    #plt.xlabel('Input feature',fontsize=13)
    ...
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,A_mean_baseline1_96_1[:,:,1])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_Salinity',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.savefig("shap_composite_8_UCDW_time_"+str(w)+".png")

    w=1
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,A_mean_baseline1_96_1[:,:,2])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_EKE',fontsize=13)
    # Set y-axis to log scale
    #plt.xlabel('Input feature',fontsize=13)
    ...
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,A_mean_baseline1_96_1[:,:,3])
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('SHAP_N',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Input feature',fontsize=13)
...
predictions=np.zeros((264,3))
predictions1=np.zeros((264,3))
predictions2=np.zeros((264,3))
predictions3=np.zeros((264,3))
predictions4=np.zeros((264,3))
predictions5=np.zeros((264,3))
predictions6=np.zeros((264,3))
predictions7=np.zeros((264,3))
predictions8=np.zeros((264,3))
predictions9=np.zeros((264,3))
...
for w in range(1):
  ...
  X_ss, Y_mm =  split_sequences(D2,T2,96,96)
  print("X_ss",X_ss.shape)
  print("y_mm",Y_mm.shape)
  train_ratio=0.7
  train_len = round(len(X_ss[:-(96+96+264)]) * train_ratio)
...
for w in range(1):
    ...
    plt.savefig('skill_total_UCDW.png')
