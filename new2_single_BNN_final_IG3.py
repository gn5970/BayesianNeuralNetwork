...
Z0_IG_24_1=np.zeros((264,members[0]))
Z1_IG_24_1=np.zeros((264,members[0]))
Z2_IG_24_1=np.zeros((264,members[0]))
Z3_IG_24_1=np.zeros((264,members[0]))
Z4_IG_24_1=np.zeros((264,members[0]))
Z5_IG_24_1=np.zeros((264,members[0]))
Z6_IG_24_1=np.zeros((264,members[0]))

Z0_IG_96_1=np.zeros((264,members[0]))
Z1_IG_96_1=np.zeros((264,members[0]))
Z2_IG_96_1=np.zeros((264,members[0]))
Z3_IG_96_1=np.zeros((264,members[0]))
Z4_IG_96_1=np.zeros((264,members[0]))
Z5_IG_96_1=np.zeros((264,members[0]))
Z6_IG_96_1=np.zeros((264,members[0]))
...
U=['data_NADW_X.mat','data_LCDW_X.mat','data_UCDW_X.mat']
U1=['data_NADW_Y.mat','data_LCDW_Y.mat','data_UCDW_Y.mat']
skill=np.zeros((9,5,40))
...
#    ts[1:] = ts_diff[1:] + ts_base[:-1]
#    return ts.tolist() 
def calculate_integrated_gradients_constbaseline(model, input_sequence, targets,num_steps=150):
...
skill=np.zeros((9,10))
IG_NADW1=np.zeros((4,264))
IG_LCDW1=np.zeros((4,264))
IG_UCDW1=np.zeros((4,264))

IG_NADW2=np.zeros((4,264))
IG_LCDW2=np.zeros((4,264))
IG_UCDW2=np.zeros((4,264))
...
for r in range(10):
 ...
 for w in range(1):
    ...
    if w ==0:
        ...
        #    T2[:,j]=ts_diff(T[:,j])    
        X_ss, Y_mm =  split_sequences(D2,T, input2[0],output2[0])
        test_len=96+96
        ...
        # Assuming x_train and y_train are your entire datasets (features and targets)
    ...
    for N in range(264):
       if N==0:
         if w ==0:
            ...
            #random_baseline = torch.randn_like(x_test)
            #baseline_sequence = random_baseline
            ...
            ig_attributions_all1.append((ig_ts1))
    ...
    Z0_IG_24_1[:,r]=ig_attributions_all1[:,-1,-72,0]
    Z1_IG_24_1[:,r]=ig_attributions_all1[:,-1,-72,1]
    Z2_IG_24_1[:,r]=ig_attributions_all1[:,-1,-72,2]
    Z3_IG_24_1[:,r]=ig_attributions_all1[:,-1,-72,3]

    Z0_IG_96_1[:,r]=ig_attributions_all1[:,-1,-1,0]
    Z1_IG_96_1[:,r]=ig_attributions_all1[:,-1,-1,1]
    Z2_IG_96_1[:,r]=ig_attributions_all1[:,-1,-1,2]
    Z3_IG_96_1[:,r]=ig_attributions_all1[:,-1,-1,3]            

    A_mean_baseline1_24_1[:,r,:]=np.concatenate((Z0_IG_24_1[:,r,np.newaxis],Z1_IG_24_1[:,r,np.newaxis],Z2_IG_24_1[:,r,np.newaxis],Z3_IG_24_1[:,r,np.newaxis]),axis=1)
    A_mean_baseline1_96_1[:,r,:]=np.concatenate((Z0_IG_96_1[:,r,np.newaxis],Z1_IG_96_1[:,r,np.newaxis],Z2_IG_96_1[:,r,np.newaxis],Z3_IG_96_1[:,r,np.newaxis]),axis=1)
...
for i in range(10):
    ...
    plt.plot(years[-264:],prediction1[::-1,i],label = 'tcn',color='g',linewidth=0.5)
    plt.plot(years[-264:],np.mean(prediction1[::-1,:],axis=1),label = 'tcn',color='k',linewidth=1.75)
    plt.plot(years[-264:],T[-264:,], label = 'real',color='r',linewidth=1.75)
    plt.ylabel("NO3 anomaly",fontsize=13)
    plt.xlabel("Years",fontsize=13)
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)

plt.savefig('prediction_8_UCDW_'+str(w)+'.pdf')

plt.figure()
for i in range(10):
    w=0
    #plt.fill_between(years[-264:],(prediction1[::-1]-ci), (prediction1[::-1]+ci), color='green', alpha=0.5)
    plt.plot(years[-264-72:-72],prediction7[::-1,i],label = 'tcn',color='g',linewidth=0.5)
    plt.plot(years[-264-72:-72],np.mean(prediction7[::-1,:],axis=1),label = 'tcn',color='k',linewidth=1.75)
    plt.plot(years[-264-72:-72],T[-264-72:-72,], label = 'real',color='r',linewidth=1.75)
    plt.ylabel("NO3 anomaly",fontsize=13)
    plt.xlabel("Years",fontsize=13)
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)

plt.savefig('prediction_2_UCDW_'+str(w)+'.pdf')
...
for w in range(1):
    ...
    plt.savefig("ig_composite_2_UCDW_"+str(w)+".png")
...
for w in range(1):
    ...
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.savefig("ig_composite_8_UCDW_"+str(w)+".png")
...
for w in range(1):
    ...
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,np.nanmean(A_mean_baseline1_24_1[:,:,0],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_PT',fontsize=13)
    # Set y-axis to log scale
    #plt.xlabel('Input feature',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,np.nanmean(A_mean_baseline1_24_1[:,:,1],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_Salinity',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.savefig("ig_composite_2_UCDW_time_"+str(w)+".png")
    w=1
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,np.nanmean(A_mean_baseline1_24_1[:,:,2],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_EKE',fontsize=13)
    # Set y-axis to log scale
    #plt.xlabel('Input feature',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,np.nanmean(A_mean_baseline1_24_1[:,:,3],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_N',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    ...
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,np.nanmean(A_mean_baseline1_96_1[:,:,0],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_PT',fontsize=13)
    # Set y-axis to log scale
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,np.nanmean(A_mean_baseline1_96_1[:,:,1],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_Salinity',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
    #plt.ylabel('feature',fontsize=13)
    #plt.xlabel('composite_shap',fontsize=13)
    plt.savefig("ig_composite_8_UCDW_time_"+str(w)+".png")
    
    w=1
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(years,np.nanmean(A_mean_baseline1_96_1[:,:,2],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_EKE',fontsize=13)
    # Set y-axis to log scale
    #plt.xlabel('Input feature',fontsize=13)
    ...
    #plt.xlabel('composite_shap',fontsize=13)
    plt.subplot(2,1,2)
    plt.plot(years,np.nanmean(A_mean_baseline1_96_1[:,:,3],axis=1))
    #shap.summary_plot(A_mean_baseline1,feature_names=feature,plot_type="auto")
    plt.ylabel('IG_N',fontsize=13)
    # Set y-axis to log scale
    plt.xlabel('Time',fontsize=13)
...
for w in range(1):
    ...
    plt.savefig('skill_total_UCDW_'+str(w)+'.png')
