...
for r in range(10):
 ...
 for w in range(1):
    ...
    if w ==0:
        ...
        #torch.cuda.manual_seed_all(seed)
        data = io.loadmat('data_LCDW.mat')
        # Extract variables X and y

        thetao_index_LCDW = data['thetao_index_LCDW']
        thetao_index_LCDW=thetao_index_LCDW.reshape(1980,1)
        so_index_LCDW = data['so_index_LCDW']
        so_index_LCDW=so_index_LCDW.reshape(1980,1)
        eke_index_LCDW = data['eke_index_LCDW']
        eke_index_LCDW=eke_index_LCDW.reshape(1980,1)
        N_index_LCDW = data['N_index_LCDW']
        N_index_LCDW=N_index_LCDW.reshape(1980,1)

        T = data['T']
        D=np.concatenate((thetao_index_LCDW,so_index_LCDW,eke_index_LCDW,N_index_LCDW),axis=1)
        T=T.reshape(1980,1) 
        D2=np.zeros((1980,4))
...
for w in range(1):
    ...
    plt.savefig('skill_total_LCDW.png')
